...
- **Packs** (`src/packs/`) — a "Pack" is a self-contained bundle of Agents (and, eventually, their own Tools) for one product or domain. Seven exist now: `core-demo` (placeholders, proves the pattern), `personal-assistant` (real-world admin/customer-service drafting, ADR 0005), `dispatcher` (plans and chains multi-agent workflows, ADR 0008), `career-advisor` (medical career/application prep, ADR 0009), `ai-research` (case report drafting, ADR 0010), `job-search` (job matching and resume tailoring, ADR 0012 — the first Pack outside medicine), and `public-agent-creation` (drafts a reviewable design for a public-facing Agent — purpose, prompt, least-privilege tools, tests, publish checklist — and never publishes anything itself). IM Brain remains unbuilt.
...
- **Config** (`src/config/load.ts`) — the "wiring" step that runs when the program starts: it registers the built-in Providers and Tools, then loads all seven current Packs (`core-demo`, `personal-assistant`, `dispatcher`, `career-advisor`, `ai-research`, `job-search`, and `public-agent-creation`).
...
- **CLI** (`src/cli/index.ts`) — the command-line program you actually run. `orchestrator help` prints the authoritative, current list; the main groups are `run` / `list-agents` / `status` (run a task, list agents, snapshot the project), `inkbox` and `browser` (email and web access), `dispatch` (plain-English goals chained across agents), `jobs` (the scheduled job-search pipeline below), `dashboard`, and the coordination commands (`coworker`, `agent-status`, `recommend`, `operational-update`, `constraints`).
- **Scheduled job-search pipeline** (`src/jobsearch/`, driven by `orchestrator jobs ...`; ADR 0014–0017, plan in `PLAN.md`) — separate from the `job-search` Pack's chat agent. Once a day it fetches postings from public job-board feeds and forwarded LinkedIn/Indeed alert emails, removes duplicates, filters with fixed rules, scores what's left with a small Claude model, and writes a digest per person ("profile") — currently two, whose data is kept strictly apart. Optional, individually switched-on steps text/iMessage/email the digest, and a feedback loop reads the candidate's direct replies and applies preference changes she asks for. The command code is split by job: `jobs-commands.ts` (which subcommand, which profile), `jobs-delivery.ts` (sending the digest), `jobs-feedback.ts` (the reply loop), and `jobs-context.ts` (settings and clients, which tests replace with fakes so nothing real is ever contacted).
...
- **Test important behavior.** Important behavior is covered by automated tests, not just claimed to work — run `npm test` for the current count rather than trusting a number written here.
