...
test("runCli status reports agent/provider/tool/pack counts and names", async () => {
  ...
  assert.equal(code, 0);
  const output = stdout.join("\n");
  assert.match(output, /Agents:\s+6 \(default, demo, inkbox-send, personal-admin, dispatcher, career-advisor\)/);
  ...
  assert.match(output, /Packs:\s+4 \(core-demo, personal-assistant, dispatcher, career-advisor\)/);
