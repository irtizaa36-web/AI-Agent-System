...
test("runCli status reports agent/provider/tool/pack counts and names", async () => {
  ...
  assert.equal(code, 0);
  const output = stdout.join("\n");
  assert.match(output, /Agents:\s+13 \(default, demo, inkbox-send, personal-admin, dispatcher, career-advisor, case-report-writer, job-search-agent, public-agent-builder, polymarket-trader, sleeper-manager, pickem-researcher, settlements-agent\)/);
