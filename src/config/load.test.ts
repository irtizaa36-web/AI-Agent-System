...
test("loadDefaultConfig wires up engine providers/tools plus the enabled packs' agents", () => {
  ...
  const names = registry
    ...
    .sort();
  assert.deepEqual(names, ["career-advisor", "case-report-writer", "default", "demo", "dispatcher", "inkbox-send", "job-search-agent", "personal-admin", "pickem-researcher", "polymarket-trader", "public-agent-builder", "settlements-agent", "sleeper-manager"]);
  assert.deepEqual(registry.listPacks(), ["core-demo", "personal-assistant", "dispatcher", "career-advisor", "ai-research", "job-search", "public-agent-creation", "prediction-markets", "sleeper", "settlements"]);
});
...
test("dispatchableAgents excludes the dispatcher itself and utility/demo agents, keeping only agents with a description", () => {
  ...
  const names = dispatchableAgents(registry)
    .map((a) => a.name)
    .sort();
  assert.deepEqual(names, ["career-advisor", "case-report-writer", "default", "job-search-agent", "personal-admin", "pickem-researcher", "polymarket-trader", "public-agent-builder", "settlements-agent", "sleeper-manager"]);
});
